...
void write3D(FILE* output_file, float*** array, int hight, int width, int channels)
{
...
void write1D(FILE* output_file, float* array, int length)
{
...
int main()
{
    ...
    FILE* conv_layer1_content = fopen("debug/conv_layer1_c.out","w");
    ...
    FILE* pool_layer1_content = fopen("debug/pool_layer1_c.out","w");
    ...
    FILE* conv_layer2_content = fopen("debug/conv_layer2_c.out","w");
    ...
    FILE* pool_layer2_content = fopen("debug/pool_layer2_c.out","w");
    ...
    FILE* flatten_content = fopen("debug/flatten_c.out","w");
    ...
    FILE* fc_layer1_content = fopen("debug/fc_layer1_c.out","w");
    ...
    FILE* fc_layer2_content = fopen("debug/fc_layer2_c.out","w");
    ...
    FILE* fc_layer3_content = fopen("debug/fc_layer3_c.out","w");
    ...
    write3D(conv_layer1_content,conv_layer1_activation,A1_SIZE,A1_SIZE,A1_CHANNELS);
    write3D(pool_layer1_content,pool_layer1_out,P1_SIZE,P1_SIZE,P1_CHANNELS);

    write3D(conv_layer2_content,conv_layer2_activation,A2_SIZE,A2_SIZE,A2_CHANNELS);
    write3D(pool_layer2_content,pool_layer2_out,P2_SIZE,P2_SIZE,P2_CHANNELS);

    write1D(flatten_content,flatten_layer,FLATTEN_SIZE);

    write1D(fc_layer1_content,fc_layer1_activation,FC1_ACT_SIZE);
    write1D(fc_layer2_content,fc_layer2_activation,FC2_ACT_SIZE);
    write1D(fc_layer3_content,fc_layer3_activation,FC3_ACT_SIZE);

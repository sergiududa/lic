...
void conv(float output[A_SIZE][A_SIZE][A_CHANNELS], float image[IMAGE_SIZE][IMAGE_SIZE][IMAGE_CHANNELS], float weight[CONV_KERNEL_SIZE][CONV_KERNEL_SIZE][CONV_CHANNELS][CONV_FILTERS], float bias[CONV_BIAS_SIZE])
{
	...
	for(filter = 0; filter < CONV_FILTERS; filter++)
	{
		...
		conv_label4:for(i = 0; i < (IMAGE_SIZE - CONV_KERNEL_SIZE + 1); i += CONV_STRIDE)
			conv_label3:for(j = 0; j < (IMAGE_SIZE - CONV_KERNEL_SIZE + 1); j += CONV_STRIDE)
			{
				//Start the convolution using the determined i & j
				sum = 0;
				conv_label0:for(row_offset = 0; row_offset <CONV_KERNEL_SIZE; row_offset++)

					conv_label2:for(col_offset = 0; col_offset <CONV_KERNEL_SIZE; col_offset++)
						conv_label1:for(channel_offset = 0; channel_offset < CONV_CHANNELS; channel_offset++)

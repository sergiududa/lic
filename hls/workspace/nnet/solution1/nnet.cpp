...
void conv_layer1(float24_t output[A1_SIZE][A1_SIZE][A1_CHANNELS], float24_t image[IMAGE_SIZE][IMAGE_SIZE][IMAGE_CHANNELS], float24_t weight[CONV1_KERNEL_SIZE][CONV1_KERNEL_SIZE][CONV1_CHANNELS][CONV1_FILTERS], float24_t bias[CONV1_BIAS_SIZE])
{
	...
	conv_layer1_label19:for(filter = 0; filter < CONV1_FILTERS; filter++)
	{
		// Pin point the the top left corner matrix that is TBC
		conv_layer1_label8:for(i = 0; i < (IMAGE_SIZE - CONV1_KERNEL_SIZE + 1); i += CONV1_STRIDE)
			conv_layer1_label7:for(j = 0; j < (IMAGE_SIZE - CONV1_KERNEL_SIZE + 1); j += CONV1_STRIDE)
			{
				...
				conv_layer1_label6:for(row_offset = 0; row_offset <CONV1_KERNEL_SIZE; row_offset++)
					...
					conv_layer1_label5:for(col_offset = 0; col_offset <CONV1_KERNEL_SIZE; col_offset++)
...
void conv_layer2(float24_t output[A2_SIZE][A2_SIZE][A2_CHANNELS], float24_t image[P1_SIZE][P1_SIZE][P1_CHANNELS], float24_t weight[CONV2_KERNEL_SIZE][CONV2_KERNEL_SIZE][CONV2_CHANNELS][CONV2_FILTERS], float24_t bias[CONV2_BIAS_SIZE])
{
	...
	{
		// Pin point the the top left corner matrix that is TBC
		conv_layer2_label12:for(i = 0; i < (P1_SIZE - CONV2_KERNEL_SIZE + 1); i += CONV2_STRIDE)
			conv_layer2_label11:for(j = 0; j < (P1_SIZE - CONV2_KERNEL_SIZE + 1); j += CONV2_STRIDE)
			{
				//Start the convolution using the determined i & j
				sum = 0;
				conv_layer2_label10:for(row_offset = 0; row_offset <CONV2_KERNEL_SIZE; row_offset++)

					conv_layer2_label9:for(col_offset = 0; col_offset <CONV2_KERNEL_SIZE; col_offset++)
						conv_layer2_label1:for(channel_offset = 0; channel_offset < CONV2_CHANNELS; channel_offset++)
							sum += image[i + row_offset][j + col_offset][channel_offset] * weight[row_offset][col_offset][channel_offset][filter];
...
void pool_layer1(float24_t output[P1_SIZE][P1_SIZE][P1_CHANNELS], float24_t image[A1_SIZE][A1_SIZE][A1_CHANNELS])
{
	...
	for(int channel = 0; channel < A1_CHANNELS; channel++)
		pool_layer1_label15:for(int i = 0; i < A1_SIZE -P1_KERNEL_SIZE + 1; i += P1_STRIDE)
			pool_layer1_label14:for(int j = 0; j < A1_SIZE - P1_KERNEL_SIZE + 1; j += P1_STRIDE)
			{
				max = image[i][j][channel];
				pool_layer1_label13:for(int k = 0; k < P1_KERNEL_SIZE; k++)
					pool_layer1_label2:for(int l = 0; l <  P1_KERNEL_SIZE; l++)
...
void pool_layer2(float24_t output[P2_SIZE][P2_SIZE][P2_CHANNELS], float24_t image[A2_SIZE][A2_SIZE][A2_CHANNELS])
{
	...
	for(int channel = 0; channel < A2_CHANNELS; channel++)
		pool_layer2_label18:for(int i = 0; i < A2_SIZE -P2_KERNEL_SIZE + 1; i += P2_STRIDE)
			pool_layer2_label17:for(int j = 0; j < A2_SIZE - P2_KERNEL_SIZE + 1; j += P2_STRIDE)
			{
				max = image[i][j][channel];
				pool_layer2_label16:for(int k = 0; k < P2_KERNEL_SIZE; k++)
					pool_layer2_label3:for(int l = 0; l <  P2_KERNEL_SIZE; l++)
						max = image[k + i][l + j][channel] > max ? image[k + i][l + j][channel] : max;

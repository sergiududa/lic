#define IMAGE_SIZE 			32
#define IMAGE_CHANNELS		3

#define CONV1_KERNEL_SIZE 	2
#define CONV1_CHANNELS 		3
#define CONV1_FILTERS 		6
#define CONV1_BIAS_SIZE 	6
#define CONV1_STRIDE 		1

#define CONV2_KERNEL_SIZE 	3
#define CONV2_CHANNELS 		6
#define CONV2_FILTERS 		4
#define CONV2_BIAS_SIZE		4
#define CONV2_STRIDE 		1

#define A1_SIZE				31
#define A1_CHANNELS			6


#define P1_SIZE				15
#define P1_CHANNELS			6
#define P1_KERNEL_SIZE		2
#define P1_STRIDE			2

#define A2_SIZE				13
#define A2_CHANNELS			4

#define P2_SIZE				6
#define P2_CHANNELS			4
#define P2_KERNEL_SIZE		2
#define P2_STRIDE			2

#define FLATTEN_SIZE		144

#define FC1_WEIGHTS_H		576
#define FC1_WEIGHTS_W		120
#define FC1_BIAS_SIZE		120

#define FC2_WEIGHTS_H		120
#define FC2_WEIGHTS_W		84
#define FC2_BIAS_SIZE		84

#define FC3_WEIGHTS_H		84
#define FC3_WEIGHTS_W		10
#define FC3_BIAS_SIZE		10

#define FC1_ACT_SIZE		120
#define FC2_ACT_SIZE		84
#define FC3_ACT_SIZE		10


from tensorflow.examples.tutorials.mnist import input_data
import numpy as np
import tensorflow as tf
import math
from sklearn.utils import shuffle
from scipy.io import savemat
from tensorflow.python.framework import ops


def save_3D(W1,name):
    W1_numpy = np.empty((0), dtype = float)
    n = W1.shape[2]
    for i in range (0,n):
        aux = W1[ : ,: , i]
        W1_numpy = np.append(W1_numpy,aux)
        
    np.savetxt(name,W1_numpy,fmt = '%.6f', delimiter=' ') 

def save_4D(W1,name):
    W1_numpy = np.empty((0), dtype = float)
    f = W1.shape[3]
    c = W1.shape[2]
    m = W1.shape[1]
    n = W1.shape[0]
    for i in range (0,f):
        for j in range (0,c):
            aux = W1[ : ,: , j, i]
            W1_numpy = np.append(W1_numpy,aux)
    np.savetxt(name,W1_numpy,fmt = '%.6f', delimiter=' ') 



#Loading Mnist Dataset
'''
mnist = input_data.read_data_sets("MNIST_data", one_hot = True, reshape=False)

#Train set
X_dataset, y_dataset = mnist.train.images, mnist.train.labels
X_dataset = np.pad(X_dataset, ((0,0),(2,2),(2,2),(0,0)), 'constant')
'''
#X_test = X_dataset[0,:,:,:]
#X_test = np.reshape(X_test, (1,32,32,1))

# PlaceHolder
#X = tf.placeholder(tf.float32, (None, 32, 32, 1))

#Initiliaze params

tf.set_random_seed(1)          
        
W1 =  tf.get_variable("W1", [2,2,3,6], initializer = tf.contrib.layers.xavier_initializer(seed = 0))
b1 =  tf.get_variable("b1", [6], initializer = tf.contrib.layers.xavier_initializer(seed = 0))
W2 =  tf.get_variable("W2", [3,3,6,4], initializer = tf.contrib.layers.xavier_initializer(seed = 0))
b2 =  tf.get_variable("b2", [4], initializer = tf.contrib.layers.xavier_initializer(seed = 0))


X =  tf.get_variable("X", [1,32,32,3], initializer = tf.contrib.layers.xavier_initializer(seed = 0))

   
conv_filters = { "conv1_s" : 1,
                 "conv2_s" : 1}

pool_filters = { "pool1_s" : 2,
                 "pool1_f" : 2,
                 "pool2_s" : 2,
                 "pool2_f" : 2}

Z1 = tf.nn.conv2d(X, W1, strides = [1, conv_filters["conv1_s"], conv_filters["conv1_s"], 1], padding = 'VALID')# + b1;
Z1 = tf.nn.bias_add(Z1, b1)
A1 = tf.nn.relu(Z1)

P1 = tf.nn.max_pool(A1, ksize = [1, pool_filters["pool2_f"], pool_filters["pool2_f"], 1], strides = [1, pool_filters["pool2_s"], pool_filters["pool2_s"], 1], padding = 'VALID')


Z2 = tf.nn.conv2d(P1, W2, strides = [1, conv_filters["conv2_s"], conv_filters["conv2_s"], 1], padding = 'VALID')# + b1;
Z2 = tf.nn.bias_add(Z2, b2)
A2 = tf.nn.relu(Z2)

P2 = tf.nn.max_pool(A2, ksize = [1, pool_filters["pool2_f"], pool_filters["pool2_f"], 1], strides = [1, pool_filters["pool2_s"], pool_filters["pool2_s"], 1], padding = 'VALID')

P2 = tf.contrib.layers.flatten(P2)

init = tf.global_variables_initializer()
     
    # Start the session to compute the tensorflow graph
with tf.Session() as sess:
    sess.run(init)
    
    #Saving params

    W1_run = sess.run(W1)
    W2_run = sess.run(W2)


    X_run = sess.run(X)
    save_3D(X_run[0,:,:,:],'input_files/image.in')
    #save_3D(X_test,'input_files/image.in')
  
    save_4D(W1_run,'input_files/conv1_weights.out')
    save_4D(W2_run,'input_files/conv2_weights.out')
  
    b1_run = sess.run(b1)
    b2_run = sess.run(b2)
    np.savetxt('input_files/conv1_biases.out',b1_run, fmt = '%.6f', delimiter=' ') 
    np.savetxt('input_files/conv2_biases.out',b2_run, fmt = '%.6f', delimiter=' ') 


    out = sess.run(P2)
    np.savetxt('debug_py.out',out, fmt = '%.6f', delimiter=' ') 
    #save_3D(out[0,:,:,:],'debug_py.out')